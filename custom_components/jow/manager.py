...
class JowManager:
    ...
    async def async_export_week(
        ...
        # NOTE CONCEPTION : on ne VIDE PAS la collection avant remplissage.
        # Le retrait (populate collectionsIds=[]) est asynchrone côté jow
        # avec un retard variable (secondes à minutes — vérifié en réel :
        # il écrase des ajouts ultérieurs). Le populate AJOUT est fiable :
        # on ajoute les plats du planning, ceux déjà présents ne dupliquent
        # pas. Les plats retirés du planning restent en collection (une
        # collection est aussi une mémoire de ce qui a été prévu).
